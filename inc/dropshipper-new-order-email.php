<?php
    function dfm_dropshippers_delete_old_slip_folder($path){
        if(!empty($path)){
            if(is_dir($path) === true){
    ...
    function dfm_dropshippers_set_html_content_type() {
        return 'text/html';
    }
    // Check if Multidrop extension is active
    add_action('woocommerce_order_status_processing', 'dfm_send_email_to_dropshippers');
    // revert
    add_action('woocommerce_order_status_processing_to_failed', 'dfm_dropshipper_cancel_earnings');
    add_action('woocommerce_order_status_processing_to_on-hold', 'dfm_dropshipper_cancel_earnings');
    add_action('woocommerce_order_status_processing_to_pending', 'dfm_dropshipper_cancel_earnings');
    add_action('woocommerce_order_status_processing_to_refunded', 'dfm_dropshipper_cancel_earnings');
    add_action('woocommerce_order_status_processing_to_cancelled', 'dfm_dropshipper_cancel_earnings');
    // REMOVE DROPSHIPPERS EARNINGS
    function dfm_dropshipper_cancel_earnings($order_id){
        $dropshippers_earnings = get_post_meta( $order_id, 'dfm_dropshippers_earnings', true);
        if(!empty($dropshippers_earnings)){
            foreach($dropshippers_earnings as $user_id => $earnings){
                $dropshipper_earning = get_user_meta($user_id, 'dfm_dropshipper_earnings', true);
                if($dropshipper_earning){
                    $dropshipper_earning = $dropshipper_earning - $earnings;
                    update_user_meta($user_id, 'dfm_dropshipper_earnings', $dropshipper_earning);
                }
            }
        }
        delete_post_meta($order_id, 'dfm_dropshippers_earnings');
    ...
    function dfm_send_email_to_dropshippers($order_id){
        ...
        $dropshippers = get_users(array( 'role' => 'dfm_dropshipper'));
        $options = get_option('dfm_woocommerce_dropshippers_options');
        if(empty($options['send_pdf']) ){
            $options['send_pdf'] = 'Yes';
        }
        $latest_order_date = get_option('dfm_woocommerce_dropshippers_latest_order_date');
        $today = date('Y-m-d');
            ...
            if( ($todayInSeconds - $latestOrderDateInSeconds) < $twoDaysAgoInSeconds ){
                ...
                while($folderToDelete != $twoDaysAgo){
                    ...
                    if( (!empty($pathToDelete)) && is_dir($pathToDelete) ) {
                        dfm_dropshippers_delete_old_slip_folder($pathToDelete);
                    }
                    $folderToDelete = date('Y-m-d', strtotime(''. $folderToDelete . ' +1 day') );
                }
                update_option('dfm_woocommerce_dropshippers_latest_order_date', $twoDaysAgo);
            }
        ...
        if( is_array( $dropshippers ) && count( $dropshippers ) > 0 ) {
            ...
            foreach ($dropshippers as $dropshipper) {
                $dropshipper_earning = get_user_meta($dropshipper->ID, 'dfm_dropshipper_earnings', true);
                if(!$dropshipper_earning){
                ...
                foreach ($items as $item_id => $item) {
                    ...
                    if(get_post_meta( $item["product_id"], 'dfm_woo_dropshipper', true) == $user_login){
                ...
                if(!empty($real_products[$user_login])){
                    ...
                    <?php
                        if($options['send_pdf'] == 'Yes'){
                            ...
                            $pdf->Cell(10, 6, ' ');
                            $pdf->Ln();
                            $pdf->Cell(100, 6, str_replace("%NUMBER%", $order->get_order_number(), __('Order slip #%NUMBER%','woocommerce-dropshippers')));
                            $pdf->Ln();
                            ...
                            $pdf->Ln();
                            $pdf->Cell(100, 8, ' ');
                            $multicell_string = strip_tags(str_replace('<br/>', "\n", $order->get_formatted_shipping_address())) . "\n\n";
                            if($options['can_see_email'] == 'Yes'){
                                ...
                                $multicell_string .= __('Email:','woocommerce-dropshippers'). " $billing_email\n";
                            }
                            if($options['can_see_phone'] == 'Yes'){
                                ...
                                $multicell_string .= __('Tel:','woocommerce-dropshippers'). " $billing_phone\n";
                            }
                            $pdf->MultiCell(100, 5, $multicell_string);
                            $pdf->setY(max($pdf->getY(), $last_pdfY));
                            if(isset($options['can_see_customer_order_notes']) && $options['can_see_customer_order_notes'] == 'Yes'){
                                if(isset($options['can_see_customer_order_notes_on_pdf']) && ($options['can_see_customer_order_notes_on_pdf'] == 'Yes') && $post->post_excerpt ){
                                    $customer_order_notes = __( 'Customer Provided Note', 'woocommerce' ) . ":\n" . strip_tags(str_replace('<br/>', "\n", $post->post_excerpt));
                                    $pdf->Ln();
                                    $pdf->MultiCell(100, 5, $customer_order_notes);
                                    $pdf->Cell(10, 8, ' ');
                                }
                            }
                            // start table
                        ...
                        foreach ($real_products[$user_login] as $item) {
                            ...
                            $drop_price = get_post_meta( $item['product_id'], '_dfm_dropshipper_price', true );
                            ...
                            $item_meta = '';
                            if($item['variation_id'] != 0){
                                $drop_price = get_post_meta( $item['variation_id'], '_dfm_dropshipper_price', true );
                                if(!$drop_price){ $drop_price = 0;}
                                $drop_price = (float) str_replace($decimal_sep, '.', ''.$drop_price);
                        ...
                        if( !isset($options['can_add_droshipping_fee']) || ( isset($options['can_add_droshipping_fee']) && $options['can_add_droshipping_fee'] == 'Yes' ) ){
                            $customer_shipping_country = get_post_meta($order_id, '_dfm_shipping_country', true);
                            $country = get_user_meta($dropshipper->ID, 'dfm_dropshipper_country', true);
                            if(empty($country)){ $country = 'US';}
                            $nat = get_user_meta($dropshipper->ID, 'dfm_national_shipping_price', true);
                            $inter = get_user_meta($dropshipper->ID, 'dfm_international_shipping_price', true);
                            if(empty($nat)){$nat = 0;}
                        ...
                        if($drop_total_earnings){
                            update_user_meta($dropshipper->ID, 'dfm_dropshipper_earnings', ($dropshipper_earning + $drop_total_earnings));
                    ...
                    <?php
                        if(isset($options['can_see_customer_order_notes']) && $options['can_see_customer_order_notes'] == 'Yes'){
                            if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' == get_option( 'dfm_woocommerce_enable_order_comments', 'yes' ) ) && $post->post_excerpt ) {
                ...
                update_post_meta($order_id, 'dfm_dropshippers', $final_dropshippers);
            }
            update_post_meta($order_id, 'dfm_dropshippers_earnings', $dropshippers_earnings);
            update_option('dfm_woocommerce_dropshippers_one_time_keys', $one_time_keys);
        }
    }
?>