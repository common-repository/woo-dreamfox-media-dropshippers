...
function artic_parsepngstream($f)
{
    // Check signature
    if($this->_readstream($f,8)!=chr(137).'PNG'.chr(13).chr(10).chr(26).chr(10)) {/* do nothing*/}
    // Read header chunk
    $this->_readstream($f,4);
    if($this->_readstream($f,4)!='IHDR') {/* do nothing*/}
    $w = $this->_readint($f);
    $h = $this->_readint($f);
    ...
    if(ord($this->_readstream($f,1))!=0) {/* do nothing*/}
    if(ord($this->_readstream($f,1))!=0) {/* do nothing*/}
