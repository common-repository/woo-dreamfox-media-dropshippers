...
function dfm_woocommerce_dropshippers_csv_to_array($filename='', $delimiter=',', $enclosure='"')
{
...
function dfm_woocommerce_dropshippers_plugins_loaded(){
	global $pagenow;
	if( ($pagenow == 'admin.php') && isset($_POST['option_page']) && ($_POST['option_page'] == 'dropshippers_importexport') ){
		...
		if(isset($_POST['action'])){
			if(wp_verify_nonce( $_POST['_wpnonce'], 'dropshippers_importexport_nonce' )){
				switch ($_POST['action']) {
					case 'update':
						update_option('dfm_dropshippers_importexport', array(
						...
						break;
					case 'import':
						$options = get_option('dfm_dropshippers_importexport', array(
							'delimiter' => ',',
							'enclosure' => '"',
						));
						try {
							...
							$csv = dfm_woocommerce_dropshippers_csv_to_array($_FILES['import-file']['tmp_name'],$options['delimiter'],$options['enclosure']);
							...
							if(!empty($csv)){
								foreach ($csv as $rowkey => $row) {
									if(isset($row['id']) && isset($row['dropshipper']) && isset($row['stock'])){
										...
										update_post_meta( $row['id'], 'dfm_woo_dropshipper', $row['dropshipper']);
									}
								}
							}
					...
					case 'export':
						...
						while ( $the_query->have_posts() ) {
							...
							$dropshipper = get_post_meta( $id, 'dfm_woo_dropshipper', true);
...
add_action('wp_loaded', 'dfm_woocommerce_dropshippers_plugins_loaded');

function dfm_dropshippers_importexport_page(){
	if(!current_user_can('manage_woocommerce')){
		wp_die(__('You do not have sufficient permissions to access this page.'));
	}
	$options = get_option('dfm_dropshippers_importexport', array(
