...
<?php
    ...
    if ( $query->have_posts() ) {
        while ( $query->have_posts() ) {
            ...
            $order_total = 0;
            $order_dropshippers = dfm_dropshippers_get_post_meta(get_the_ID(), 'dropshippers', true);
            $tel = '';
            ...
            if( (sizeof($real_products) > 0) && ($order->get_status() == 'completed' || $order->get_status() == 'processing') ){
                ...
                foreach ($real_products as $item) {
                    ...
                    if(isset($options['text_string']) && $options['text_string'] == "Yes"){
                        $table_string .= "<li>" . $my_item_post_title . $SKU . ' ' . $my_meta . ' x' . $item['qty'] . ' ('. __('FULL PRICE:','woocommerce-dropshippers') .' <span class="dfm-toberewritten">' . wc_price(( ((float) $item['line_total']) + ((float) $item['line_tax']) )) .'</span><span class="dfm-tobereconverted" style="display:none;">'.( ((float) $item['line_total']) + ((float) $item['line_tax']) ).'</span> - '. __('MY EARNINGS:','woocommerce-dropshippers') .' <span class="dfm-toberewritten">' . wc_price((float) $drop_price*$item['qty']) .'</span><span class="dfm-tobereconverted" style="display:none;">'.((float) $drop_price*$item['qty']).'</span>)';
                    }
                    else{
                        $table_string .= "<li>" . $my_item_post_title . $SKU . ' ' . $my_meta . ' x' . $item['qty'] . '('. __('MY EARNINGS:','woocommerce-dropshippers') .' <span class="dfm-toberewritten">' . wc_price((float) $drop_price*$item['qty']) .'</span><span class="dfm-tobereconverted" style="display:none;">'.((float) $drop_price*$item['qty']).'</span>)';
                    }
                    $table_string .= '</li>' . "\n";
                ...
                $country = isset($woocommerce->countries->countries[$shipping_country])?$woocommerce->countries->countries[$shipping_country]:'';
                $table_string .= '<td class="column-columnname">' . $country .'<br/>'. $order->get_formatted_shipping_address() . '</td>';
                ...
                $table_string .= '<td class="column-columnname">';
                if(isset($options['text_string']) && $options['text_string'] == "Yes"){
                    $table_string .= __('FULL TOTAL:','woocommerce-dropshippers') .'<span class="dfm-toberewritten">' . wc_price((float) $order_total) . '</span><span class="dfm-tobereconverted" style="display:none;">'.((float) $order_total).'</span><br>';
                }
                $table_string .= __('MY EARNINGS:','woocommerce-dropshippers') . '<span class="dfm-toberewritten">' . wc_price((float) $drop_total_price) . '</span><span class="dfm-tobereconverted" style="display:none;">'.((float) $drop_total_price).'</span>';
                $table_string .= "</td>\n";
                $dropshipper_shipping_info = get_post_meta(get_the_ID(), 'dfm_dropshipper_shipping_info_'.get_current_user_id(), true);
...
?>
    <div class="wrap">
	    <p><?php echo __('Order count:','woocommerce-dropshippers') . $order_count; ?></p>
	    <?php
	    $options = get_option('dfm_woocommerce_dropshippers_options');
	    ?>
	    ...
	    <form name="month" action="" method="GET"> 
	        ...
	        <input type="hidden" name="page" value="dfm_dropshipper_order_list_page" />
