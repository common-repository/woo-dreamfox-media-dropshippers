<?php
add_action('wp_ajax_woocommerce_dropshippers_get_slip', 'dfm_woocommerce_dropshippers_get_slip');
function dfm_woocommerce_dropshippers_get_slip(){
	if( empty( $_GET['action'] ) || ! is_user_logged_in() || !check_admin_referer( $_GET['action'] ) ) {
		...
		<html>
		<head>
			<title><?php echo str_replace("%NUMBER%", $order->get_order_number(), __('Order slip #%NUMBER%','woocommerce-dropshippers')); ?></title>
			<style type="text/css">
				body{
					font-family: Arial;
				}
